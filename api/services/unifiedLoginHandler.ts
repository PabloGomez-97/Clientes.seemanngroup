...
export async function handleUnifiedLogin(
  ...
  if (requestedTenant && isTenantId(requestedTenant)) {
    if (matched.includes(requestedTenant)) {
      chosen = requestedTenant;
    } else {
      // El cliente pidió un país explícito (ej. mobile Chile) pero la contraseña
      // no coincide ahí: no redirigir silenciosamente al otro país.
      return res.status(401).json({ error: 'Credenciales inválidas' });
    }
