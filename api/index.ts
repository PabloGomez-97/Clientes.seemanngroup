...
import { Ejecutivo, parseIdInterno, type IEjecutivo, type IEjecutivoDoc } from './models/Ejecutivo.js';
...
import { fetchAllExpiring } from './services/pricingExpiryService.js';
import {
  runPricingAlerts,
...
import {
  ...
  resolveDocumentMime,
} from './constants/documentFileTypes.js';
...
import {
  ...
  type ProfitMode,
} from './models/ClientProfitOverride.js';
...
async function resolveEjecutivoContext(email: string) {
  ...
  if (!ejecutivoDoc || !ejecutivoDoc._id) {
    ejecutivoDoc = await Ejecutivo.findOne({ email: String(me.email).toLowerCase().trim() });
  }
  ...
  return { me, ejecutivoDoc, isAdmin, isOperaciones, canManageAllClients, ejecutivoObjectId };
}
...
} from './models/AirConnectSpainConfig.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
      ...
      try {
        ...
        return res.json({
          ...
          ejecutivo: {
            ...
            idInterno: newEjecutivo.idInterno,
          }
        });
      } catch (e: any) {
        if (e?.message === 'No auth token' || e?.message === 'Invalid token') {
    ...
    // Crea el ejecutivo espejo en el otro país (mismo email + passwordHash).
    if (path === '/api/admin/provision-cross-tenant' && method === 'POST') {
      try {
        ...
        const result = await provisionRemoteEjecutivo({
          ...
          idInterno:
            typeof ejDoc?.idInterno === 'number' ? ejDoc.idInterno : null,
          roles: ejDoc?.roles || {
            administrador: false,
            pricing: false,
            ejecutivo: true,
    ...
    // POST /api/shipsgo/webhooks/air - fallback (ruta dedicada: api/shipsgo/webhooks/air.ts)
    if (path === '/api/shipsgo/webhooks/air' && method === 'POST') {
      console.log('🔔 [shipsgo-webhook] Received air webhook event (via api/index fallback)');
      try {
        ...
        const payload = rawBody
          ? parseShipsgoWebhookJson(rawBody)
    ...
    // POST /api/shipsgo/webhooks/ocean - fallback (ruta dedicada: api/shipsgo/webhooks/ocean.ts)
    if (path === '/api/shipsgo/webhooks/ocean' && method === 'POST') {
      console.log('🔔 [shipsgo-webhook] Received ocean webhook event (via api/index fallback)');
      try {
        const signature = req.headers['x-shipsgo-webhook-signature'] as string | undefined;
        const webhookId = req.headers['x-shipsgo-webhook-id'] as string | undefined;
        const webhookName = req.headers['x-shipsgo-webhook-name'] as string | undefined;
    ...
    if (path === '/api/documents/counts' && method === 'GET') {
      try {
        const currentUser = requireAuth(req);
    ...
    if (path === '/api/documents/all' && method === 'GET') {
      try {
        ...
        const [groundDocs, quoteDocs] = await Promise.all([
    ...
    // PUT /api/air-connect-spain/config — solo administrador
    if (path === '/api/air-connect-spain/config' && method === 'PUT') {
      try {
        const currentUser = requireAuth(req);
        const ejecutivoDoc = await Ejecutivo.findOne({ email: currentUser.sub });
        if (!ejecutivoDoc?.roles?.administrador) {
    ...
    if (path === '/api/gestion-cotizador/config' && method === 'GET') {
      try {
        ...
        return res.json({
          ...json,
          profitMarkup: normalizeGlobalProfitMarkup(
            (json as any).profitMarkup,
          ),
        });
      } catch (e) {
    ...
    if (path === '/api/gestion-cotizador/config' && method === 'PUT') {
      try {
        ...
        const body = typeof req.body === 'string' ? JSON.parse(req.body) : req.body;
        const { fcl, lcl, aereo, profitMarkup } = body as {
    ...
    // GET /api/profit-markup/effective?clientUserId=
    if (path === '/api/profit-markup/effective' && method === 'GET') {
      try {
        const currentUser = requireAuth(req);
        const rawQs = (url || '').includes('?') ? (url || '').split('?')[1] : '';
        const qs = new URLSearchParams(rawQs);
        const clientUserIdParam = qs.get('clientUserId');
        ...
        if (!targetClientId) {
          // Portal cliente: usar el usuario autenticado si no es ejecutivo
          const me = await User.findOne({ email: currentUser.sub });
          if (me && me.username !== 'Ejecutivo') {
            targetClientId = String(me._id);
          }
        } else {
          // Lectura para cotizar: cualquier ejecutivo autenticado puede resolver
          // el markup efectivo de un cliente (la mutación sigue restringida).
          const me = await User.findOne({ email: currentUser.sub });
          if (!me || me.username !== 'Ejecutivo') {
            return res.status(403).json({ error: 'No autorizado' });
          }
    ...
    // GET /api/client-profit-overrides/:clientUserId/audit
    {
      const auditMatch = path.match(
        /^\/api\/client-profit-overrides\/([^/]+)\/audit$/,
      );
      if (auditMatch && method === 'GET') {
        try {
          const currentUser = requireAuth(req);
          const clientUserId = decodeURIComponent(auditMatch[1]);
    ...
    // PUT /api/client-profit-overrides/:clientUserId
    // DELETE /api/client-profit-overrides/:clientUserId
    // DELETE /api/client-profit-overrides/:clientUserId/:mode
    {
      const overrideMatch = path.match(
        /^\/api\/client-profit-overrides\/([^/]+)(?:\/(air|fcl|lcl))?$/,
      );
      if (
        overrideMatch &&
        ...
        try {
          const currentUser = requireAuth(req);
          const clientUserId = decodeURIComponent(overrideMatch[1]);
          const modeParam = overrideMatch[2] as ProfitMode | undefined;
          const { client, isAdmin, ejecutivoDoc } =
            await assertCanManageClientProfit(currentUser.sub, clientUserId);
          ...
          if (modeParam) {
            return res.status(400).json({
              error: 'Usa PUT sobre /api/client-profit-overrides/:clientUserId sin modo',
            });
          }
          ...
          if (e?.status) return res.status(e.status).json({ error: e.message });
          console.error('[client-profit-overrides] mutate error:', e);
    ...
    // GET /api/pricing/alert-status — estado del sistema de alertas
    if (path === '/api/pricing/alert-status' && method === 'GET') {
      try {
        const currentUser = requireAuth(req);
        const ejDoc = await Ejecutivo.findOne({ email: currentUser.sub.toLowerCase() });
        if (!ejDoc?.roles?.administrador && !ejDoc?.roles?.pricing) {
    ...
    // GET /api/pricing/alert-recipients — destinatarios pricing activos
    if (path === '/api/pricing/alert-recipients' && method === 'GET') {
      try {
        const currentUser = requireAuth(req);
        const ejDoc = await Ejecutivo.findOne({ email: currentUser.sub.toLowerCase() });
    ...
    // GET /api/pricing/alert-preview — vista previa HTML sin enviar
    if (path === '/api/pricing/alert-preview' && method === 'GET') {
      try {
        const currentUser = requireAuth(req);
        const ejDoc = await Ejecutivo.findOne({ email: currentUser.sub.toLowerCase() });
        if (!ejDoc?.roles?.administrador) {
        ...
        if (!preview) {
          return res.json({
            success: true,
            hasData: false,
            message: 'No hay tarifas en la ventana seleccionada',
          });
        }
    ...
    if (path === '/api/pricing/send-alerts' && method === 'POST') {
      try {
        ...
        const result = await runPricingAlerts({
          ...
          testRecipientEmail: mode === 'test' ? currentUser.sub.toLowerCase() : undefined,
          triggeredByEmail: currentUser.sub.toLowerCase(),
        });
