/**
 * Template HTML para notificación de cotización u operación aérea.
...
function isOperacion(data: AirQuoteEmailData): boolean {
  return data.tipoAccion === 'operacion';
}

function tipoTitulo(data: AirQuoteEmailData): string {
  return isOperacion(data) ? 'Nueva operación aérea' : 'Nueva cotización aérea';
}

function badgeLine1(data: AirQuoteEmailData): string {
  return isOperacion(data) ? 'Operación' : 'Cotización';
}

function badgeLine2(): string {
  return 'Aéreo';
}
...
function formatClienteLine(data: AirQuoteEmailData): string {
...
function formatTotal(data: AirQuoteEmailData): string {
...
export function buildAirQuoteEmailHTML(data: AirQuoteEmailData): string {
  ...
  const badgeLine2Text = badgeLine2();
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:${C.card};border-radius:12px;border:1px solid ${C.border};">
                <tr>
                  <td class="route-col" width="42%" align="left" style="padding:22px 20px 22px 24px;">
                    <p style="margin:0 0 6px;font-family:${FONT};font-size:10px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:${C.muted};">Origen</p>
                    <p style="margin:0;font-family:${FONT};font-size:17px;font-weight:600;color:${C.text};line-height:1.3;">${escapeHtml(data.origen || '—')}</p>
                  </td>
                  ...
                  <td class="route-col" width="42%" align="right" style="padding:22px 24px 22px 20px;">
                    <p style="margin:0 0 6px;font-family:${FONT};font-size:10px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:${C.muted};">Destino</p>
                    <p style="margin:0;font-family:${FONT};font-size:17px;font-weight:600;color:${C.text};line-height:1.3;">${escapeHtml(data.destino || '—')}</p>
                  </td>
                </tr>
              </table>
