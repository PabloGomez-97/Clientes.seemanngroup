// server/services/linbisAuthService.ts
...
class LinbisAuthService {
  ...
  private static generatePKCE(): { codeVerifier: string; codeChallenge: string } {
    // Generar code_verifier (random 32 bytes)
  ...
  private static async performBrowserLogin(
    ...
    const browser = await puppeteer.launch({
        headless: false, // Déjalo en false para ver el proceso (puedes cambiarlo a true después)
        args: [
        '--no-sandbox',
    ...
    try {
        ...
        // Esperar a que aparezcan los campos con los IDs correctos
        await page.waitForSelector('#signInName', { timeout: 15000 });
        await page.waitForSelector('#password', { timeout: 15000 });
        console.log('[LinbisAuth] ✓ Formulario de login encontrado');
