...
import { buildDocR2Key, buildQuoteDocR2Key, uploadDocument, downloadDocumentBuffer, deleteDocument } from '../api/services/r2DocumentStorage.ts';
import {
...
function isMobileClient(req: express.Request, body?: unknown): boolean {
...
const auth: express.RequestHandler = (req, res, next) => {
  ...
  try {
    ...
    // Tokens MX: rechazar en APIs de negocio; /api/me responde 409+redirect
    const isMe = req.path === '/api/me' || req.path.endsWith('/me');
    if (decoded.tenant === 'mx' && !isMe) {
      return res.status(401).json({ error: 'Invalid token' });
    }
...
app.get('/api/mobile/notification-preferences', auth, async (req, res) => {
  try {
    const currentUser = (req as any).user as AuthPayload;
    ...
    console.error('[mobile-notification-preferences:get]', e);
    return res.status(500).json({ error: 'Error interno' });
  }
});

app.put('/api/mobile/notification-preferences', auth, async (req, res) => {
  try {
    const currentUser = (req as any).user as AuthPayload;
    ...
    console.error('[mobile-notification-preferences:put]', e);
    return res.status(500).json({ error: 'Error interno' });
  }
});

app.put('/api/mobile/push-token', auth, async (req, res) => {
  try {
    const currentUser = (req as any).user as AuthPayload;
    const pushToken = String((req.body as any)?.token || '').trim();
    const platform = String((req.body as any)?.platform || '').trim() || undefined;
    ...
    console.error('[mobile-push-token:put]', e);
    return res.status(500).json({ error: 'Error interno' });
  }
});
...
app.delete('/api/mobile/push-token', auth, async (req, res) => {
  try {
    ...
    console.error('[mobile-push-token:delete]', e);
    return res.status(500).json({ error: 'Error interno' });
  }
});
...
// POST /api/documentos/operacionales/upload - Subir documento operacional de cotización
app.post('/api/documentos/operacionales/upload', auth, async (req, res) => {
  try {
    const currentUser = (req as any).user;
    ...
    const fileBuffer = Buffer.from(base64Content, 'base64');
    const docId = new mongoose.Types.ObjectId();
    const r2Subfolder = getOperacionalR2Subfolder(modo);
    const r2Key = buildQuoteDocR2Key(
      ownerUsername,
      String(quoteNumber),
      r2Subfolder,
    ...
    console.log(`[documentos-operacionales] Documento subido: ${nuevoDocumento._id}`);
...
app.get('/api/documentos/operacionales/:quoteNumber', auth, async (req, res) => {
  try {
    const currentUser = (req as any).user;
...
app.get('/api/documentos/operacionales/download/:documentoId', auth, async (req, res) => {
  try {
    const currentUser = (req as any).user;
    ...
    if (documento.r2Key) {
      try {
        ...
        console.error('[documentos-operacionales] Error al descargar de R2:', r2Err);
      }
    }
...
app.delete('/api/documentos/operacionales/:documentoId', auth, async (req, res) => {
  try {
    const currentUser = (req as any).user;
...
app.post('/api/air-connect-spain/quotation/calculate', auth, async (req, res) => {
  try {
    const { callAirConnectQuotationApi } = await import(
      '../api/airConnectSpainQuotationProxy.ts'
    );
    const data = await callAirConnectQuotationApi(req.body);
    return res.json(data);
  } catch (e: unknown) {
