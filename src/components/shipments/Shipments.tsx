...
function ShipmentsView({ accessToken, onLogout }: ShipmentsViewProps) {
  ...
  const filterConsignee = 'MAURICIO HOCHSCHILD INGENIERÍA Y SERVICIOS SPA.';
  ...
  // Obtener shipments usando el token con paginación
  const fetchShipments = async (startPage: number = 1, append: boolean = false) => {
    if (!accessToken) {
    ...
    try {
      let allFiltered: Shipment[] = append ? [...shipments] : [];
      const pagesToLoad = 10; // Cargar 10 páginas automáticamente
      ...
      for (let page = startPage; page < startPage + pagesToLoad; page++) {
        ...
        // Filtrar directamente por consignee.name
        const filtered = shipmentsArray.filter(shipment => 
          shipment.consignee?.name === filterConsignee
        );
        ...
        console.log(`Página ${page}: ${shipmentsArray.length} shipments totales, ${filtered.length} del consignee, ${allFiltered.length} acumulados`);
        
        if (shipmentsArray.length < 100) {
          setHasMore(false);
          break;
        }
      }
      ...
      if (!append) {
        localStorage.setItem('shipmentsCache', JSON.stringify(allFiltered));
        localStorage.setItem('shipmentsCacheTimestamp', new Date().getTime().toString());
      }
      
      console.log(`Total encontrado: ${allFiltered.length} shipments de MAURICIO HOCHSCHILD`);
  ...
  const loadMorePages = async () => {
    ...
    await fetchShipments(nextPage, true);
  };
