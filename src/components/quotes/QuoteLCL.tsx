...
function getLclPackageTypeName(): string {
  return FIXED_LCL_PACKAGE_TYPE_NAME;
}

function summarizeLclPackageTypes(): string {
  return FIXED_LCL_PACKAGE_TYPE_NAME;
}
...
function buildOverallPiecesSummaryLCL(pieces: OverallPieceDataLCL[]): string {
  return pieces
    .map((piece, index) => {
      return `Pieza ${index + 1}: ${FIXED_LCL_PACKAGE_TYPE_NAME} / ${DEFAULT_OVERALL_LCL_DESCRIPTION} / ${piece.volume.toFixed(4)} m3 / ${piece.weight.toFixed(2)} kg`;
    })
    .join("; ");
}
...
export default function QuoteLCL({
  ...
  // Resetear puerto cercano si deja de ser EXW.
  useEffect(() => {
    if (incoterm !== "EXW") {
  ...
  // Resolver POL tarificado más cercano (EXW).
  useEffect(() => {
    if (
      incoterm !== "EXW" ||
      !pickupCoords ||
  ...
  useEffect(() => {
    ...
    const pods = buildPodOptionsForCountry(
      ...
      activeOriginIndex,
      activePais.value,
      (podNorm, routePod) => getPODDisplayName(podNorm) || routePod,
    );
    setOpcionesPOD(pods);
    setPodSeleccionado(null);
    setRutaSeleccionada(null);
  ...
  const generateQuotePDF = async (
    ...
    try {
      ...
      const packageTypeName = summarizeLclPackageTypes();
      ...
      await new Promise<void>((resolve) => {
        const pdfPiecesData = !overallDimsAndWeight
          ? piecesData.map((piece) => ({
            id: piece.id,
            packageTypeName: getLclPackageTypeName(),
            description: DEFAULT_OVERALL_LCL_DESCRIPTION,
            length: piece.length,
            width: piece.width,
            height: piece.height,
    ...
    return {
      ...
      commodities: overallDimsAndWeight
        ? overallPiecesData.map((piece) => ({
          ...
          packageType: {
            id: DEFAULT_OVERALL_LCL_PACKAGE_TYPE,
          },
          pieces: 1,
          description: DEFAULT_OVERALL_LCL_DESCRIPTION,
              ...
              <>
                ...
                {!isSimulationMode && routeMode === "recurrente" && (
                  <div className="mb-4">
                    <div className="row g-3 mb-4">
                      <div className="col-md-6">
                        <CountryOriginSelector
                          id="lcl-pais-recurrente"
                          label={t("Quotelcl.paisorigen", {
                    ...
                    {incoterm === "EXW" && paisSeleccionado && podSeleccionado && (
                      <div className="mb-4 bg-light p-3 rounded border">
                        {polSeleccionado && exwResolvedDistanceKm != null && (
                          <div className="alert alert-success py-2 px-3 mb-3 small">
                            <i className="bi bi-geo-alt-fill me-2"></i>
                            Puerto asignado: <strong>{polSeleccionado.label}</strong>
                            {" · "}
                        ...
                        {exwNearbyRatedPorts.length === 0 &&
                          ...
                          incoterm === "EXW" && (
                            <div className="alert alert-warning py-2 px-3 mb-3 small">
                              No hay tarifas geolocalizables para esta dirección en
