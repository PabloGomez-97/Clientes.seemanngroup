...
interface DocumentoQuoteOcean {
  id: string;
  quoteId: string;
  tipo: TipoDocumentoQuoteOcean;
...
const TIPOS_DOCUMENTO: TipoDocumentoQuoteOcean[] = [
  "Bill of Lading (BL)",
  "Facturas asociadas al servicio",
  "Endoso",
  "Invoice",
...
export const DocumentosSectionQuoteOcean: React.FC<Props> = ({
  quoteNumber,
  onCountChange,
}) => {
  ...
  const [documentos, setDocumentos] = useState<DocumentoQuoteOcean[]>([]);
  const [loading, setLoading] = useState(false);
  const [uploading, setUploading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [successMessage, setSuccessMessage] = useState<string | null>(null);
  const [carpetasAbiertas, setCarpetasAbiertas] = useState<
    Set<TipoDocumentoQuoteOcean>
  >(new Set());
  const [dragOverTipo, setDragOverTipo] =
    useState<TipoDocumentoQuoteOcean | null>(null);
  ...
  const fileInputRefs = useRef<
    Record<TipoDocumentoQuoteOcean, HTMLInputElement | null>
  >(
    Object.fromEntries(
      TIPOS_DOCUMENTO.map((tipo) => [tipo, null]),
    ) as Record<TipoDocumentoQuoteOcean, HTMLInputElement | null>,
  );
  ...
  const loadDocumentos = async () => {
    ...
    try {
      const response = await fetch(
        `/api/documentos/operacionales/${encodeURIComponent(quoteNumber)}?modo=maritimo&ownerUsername=${encodeURIComponent(ownerUsername)}`,
        {
          headers: {
  ...
  const toggleCarpeta = (tipo: TipoDocumentoQuoteOcean) => {
