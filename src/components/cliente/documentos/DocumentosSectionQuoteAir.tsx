...
interface DocumentoQuoteAir {
  id: string;
  quoteId: string;
  tipo: TipoDocumentoQuoteAir;
...
const TIPOS_DOCUMENTO: TipoDocumentoQuoteAir[] = [
  "Documento de transporte Internacional (AWB)",
  "Facturas asociados al servicio",
  "Invoice",
  "Packing List",
...
export const DocumentosSectionQuoteAir: React.FC<Props> = ({
  quoteNumber,
  onCountChange,
}) => {
  ...
  const [documentos, setDocumentos] = useState<DocumentoQuoteAir[]>([]);
  const [loading, setLoading] = useState(false);
  const [uploading, setUploading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [successMessage, setSuccessMessage] = useState<string | null>(null);
  const [carpetasAbiertas, setCarpetasAbiertas] = useState<
    Set<TipoDocumentoQuoteAir>
  >(new Set());
  const [dragOverTipo, setDragOverTipo] = useState<TipoDocumentoQuoteAir | null>(
    null,
  );
  ...
  const fileInputRefs = useRef<
    Record<TipoDocumentoQuoteAir, HTMLInputElement | null>
  >(
    Object.fromEntries(
      TIPOS_DOCUMENTO.map((tipo) => [tipo, null]),
    ) as Record<TipoDocumentoQuoteAir, HTMLInputElement | null>,
  );
  ...
  const loadDocumentos = async () => {
    if (!token || !quoteNumber || !ownerUsername) return;
    ...
    try {
      const response = await fetch(
        `/api/documentos/operacionales/${encodeURIComponent(quoteNumber)}?modo=aereo&ownerUsername=${encodeURIComponent(ownerUsername)}`,
        {
          headers: {
      ...
      console.error("Error cargando documentos operacionales:", err);
      setError(
  ...
  const toggleCarpeta = (tipo: TipoDocumentoQuoteAir) => {
  ...
  const getDocumentosPorTipo = (tipo: TipoDocumentoQuoteAir) =>
  ...
  const processFiles = async (tipo: TipoDocumentoQuoteAir, files: File[]) => {
  ...
  const handleDragEnter = (e: React.DragEvent, tipo: TipoDocumentoQuoteAir) => {
    e.preventDefault();
  ...
  const handleDrop = async (e: React.DragEvent, tipo: TipoDocumentoQuoteAir) => {
    e.preventDefault();
    setDragOverTipo(null);
  ...
  const handleDownload = async (documentoId: string, nombreArchivo: string) => {
    ...
    try {
      const response = await fetch(
        `/api/documentos/operacionales/download/${documentoId}?ownerUsername=${encodeURIComponent(ownerUsername)}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  const handleDelete = async (documentoId: string, nombreArchivo: string) => {
    ...
    try {
      const response = await fetch(
        `/api/documentos/operacionales/${documentoId}?ownerUsername=${encodeURIComponent(ownerUsername)}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
