...
function QuoteFCL({ preselectedPOL, preselectedPOD }: QuoteFCLProps = {}) {
  ...
  const generateQuotePDF = async (
    ...
    try {
      ...
      try {
        console.log(
          "[QuoteFCL-Ej] Buscando cotización recién creada (id mayor a",
          previousMaxId,
          ")...",
        );
        ...
        if (linbisRes.ok) {
          ...
          if (Array.isArray(linbisData) && linbisData.length > 0) {
            ...
            console.log(
              `[QuoteFCL-Ej] Cotización con ID más alto: number=${newestQuote.number}, id=${newestQuote.id}`,
            );
            if (Number(newestQuote.id) > (previousMaxId || 0)) {
              quoteNumber = newestQuote.number;
              console.log(
                `✅ [QuoteFCL-Ej] NUEVA COTIZACIÓN CONFIRMADA: ${quoteNumber}`,
              );
            }
          }
        }
      } catch (e) {
        console.warn("[QuoteFCL-Ej] Error obteniendo quoteNumber:", e);
      }
      ...
      await new Promise<void>((resolve) => {
        root.render(
          <PDFTemplateFCL
            ...
            remarks={rutaSeleccionada.remarks || undefined}
          />,
        );
      ...
      if (pdfElement) {
        ...
        if (pdfBase64 && quoteNumber) {
          try {
            ...
            console.log(
              "[QuoteFCL-Ej] PDF guardado en MongoDB:",
              uploadRes.status,
              uploadData,
            );
          } catch (uploadErr) {
        ...
        console.log("[QuoteFCL-Ej] PDF descargado localmente");
      }
