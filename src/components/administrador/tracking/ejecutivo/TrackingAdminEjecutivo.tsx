...
function ShipsGoTrackingAdmin() {
  ...
  const handleSelectClient = useCallback(
    (client: Cliente) => {
      ...
      navigate(`/admin/clientes/tracking/${encodeURIComponent(client.username)}`, {
        replace: true,
      });
    },
    [navigate],
  ...
  const handleBack = useCallback(() => {
    setShowCreateForm(null);
    navigate("/admin/clientes/tracking", { replace: true });
  }, [navigate]);
        ...
        <div key={`${selectedClient.username}-${trackingKey}`}>
          <ShipsGoTracking
            ...
            routeTrackingIdentifier={
              deepLinkTab ? trackingIdentifier : null
            }
            trackingRouteBase={`/admin/clientes/tracking/${encodeURIComponent(selectedClient.username)}`}
          />
        </div>
      </div>
    );
