...
import type { PricingTabParamList } from "../../navigation/PricingTabs";
import { getEjecutivoPhotoUrl, getInitials } from "../../utils/ejecutivoPhoto";
import { brand, spacing } from "../../theme/brand";
import { fonts } from "../../theme/typography";

type Nav = BottomTabNavigationProp<PricingTabParamList>;

type QuickLink = {
  key: keyof PricingTabParamList;
  label: string;
...
const QUICK: QuickLink[] = [
  { key: "Tarifario", label: "Tarifario", icon: "pricetag", accent: "orange" },
  { key: "Alertas", label: "Alertas", icon: "warning", accent: "orange" },
  {
    key: "More",
    label: "Docs proveedores",
    icon: "folder-open",
    accent: "navy",
    moreScreen: "DocsProveedores",
  },
  {
    key: "More",
    label: "Histórico",
...
const INDEX = [
  ...
  { n: "03", title: "Histórico de precios", target: "historico" as const },
  { n: "04", title: "Documentos proveedores", target: "docs" as const },
];
...
export default function PricingHomeScreen() {
  ...
  const { user } = useAuth();
  const { expiry, status, loading, refresh } = usePricingAlerts(7);
  ...
  const totals = expiry?.totals ?? { air: 0, fcl: 0, lcl: 0, all: 0 };
    ...
    () => [
      {
        label: "Por vencer",
        value: totals.all,
        sub: `Ventana ${expiry?.days ?? 7} días`,
        color: brand.primary,
      },
      {
        label: "Aéreo",
  ...
  const openIndex = (
    target: "novedades" | "promesas" | "historico" | "docs",
  ) => {
    if (target === "historico") {
    ...
    if (target === "docs") {
      navigation.navigate("More", { screen: "DocsProveedores" });
      return;
    }
    ...
    <View style={styles.root}>
      ...
      >
        ...
        >
          ...
          <View style={styles.bandTop}>
            ...
            <View style={styles.accountPill}>
              ...
              <Text style={styles.accountPillText}>Portal pricing</Text>
            </View>
          </View>
          ...
          <View style={styles.heroRow}>
            ...
            <View style={styles.heroText}>
              ...
              <Text style={styles.tagline}>
                Control de tarifas, vencimientos y proveedores
              </Text>
            </View>
          </View>
        </LinearGradient>
        ...
        <View style={styles.statusBoard}>
          ...
          >
            <Text style={styles.statusKey}>TARIFARIO</Text>
            <Text style={styles.statusVal}>Ver</Text>
        ...
        <View style={styles.sectionPad}>
          ...
          <Text style={styles.sectionTitle}>Vencimientos de tarifas</Text>
          ...
          <View style={styles.urgencyRow}>
            <Text style={styles.urgencyText}>
              Urgente: hoy {buckets?.bucketToday ?? 0} · 24h{" "}
              {buckets?.bucket24 ?? 0} · 48h {buckets?.bucket48 ?? 0}
            </Text>
          </View>
