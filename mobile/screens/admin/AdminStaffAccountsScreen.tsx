...
type StaffRow = AdminUserRow & {
  telefono?: string;
  idInterno?: number | null;
  roles?: AdminEjecutivoRow["roles"];
  activo?: boolean;
  ejecutivoDocId?: string;
};
...
export default function AdminStaffAccountsScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const byEmail = new Map(
        ejecutivos.map((ej) => [ej.email.trim().toLowerCase(), ej]),
      );
      const merged: StaffRow[] = staff.map((u) => {
        const ej = byEmail.get(u.email.trim().toLowerCase());
        return {
          ...u,
          telefono: ej?.telefono,
          idInterno: ej?.idInterno,
          roles: ej?.roles,
          activo: ej?.activo,
          ejecutivoDocId: ej?.id,
        };
      });
      setRows(
        merged.sort((a, b) =>
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) =>
      [
        ...
        r.nombreuser,
        r.telefono,
        r.idInterno != null ? String(r.idInterno) : "",
        roleLabels(r.roles),
      ]
        .filter(Boolean)
  ...
  const confirmDelete = (user: StaffRow) => {
    Alert.alert(
      "Eliminar ejecutivo",
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <View style={styles.header}>
        <Text style={styles.title}>Cuentas staff</Text>
        <Text style={styles.subtitle}>Ejecutivos y roles del portal</Text>
      </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={styles.empty}>No hay cuentas de staff.</Text>
          }
          ListFooterComponent={
            filtered.length > 0 ? (
          ...
          renderItem={({ item }) => (
            <Pressable
              style={styles.card}
              onPress={() =>
                navigation.navigate("AdminStaffAccountForm", {
                  userId: item.id,
                })
              }
            >
              ...
              {item.idInterno != null ? (
                <Text style={styles.meta}>ID interno: {item.idInterno}</Text>
              ) : null}
      ...
      <Pressable
        style={styles.fab}
        onPress={() => navigation.navigate("AdminStaffAccountForm", {})}
      >
        <Ionicons name="add" size={22} color="#fff" />
        <Text style={styles.fabText}>Nuevo ejecutivo</Text>
      </Pressable>
    </SafeAreaView>
  );
}
