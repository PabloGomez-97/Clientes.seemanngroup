...
async function fetchShipments(username: string): Promise<{
  ...
  const airRes = await fetch(`${MOBILE_API_BASE}/api/shipsgo/shipments`);
  if (airRes.ok) {
  ...
  const oceanRes = await fetch(`${MOBILE_API_BASE}/api/shipsgo/ocean/shipments`);
  if (oceanRes.ok) {
    const oceanData = await oceanRes.json();
    const ships2: Record<string, unknown>[] = Array.isArray(oceanData.shipments)
      ? oceanData.shipments
      : [];
    freshOcean = ships2
      .filter(
